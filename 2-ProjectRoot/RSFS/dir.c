...
struct dir_entry *search_dir_internal(char file_name) {
  ...
  if (root_inode == NULL) {
    ...
    root_inode->block[0] = (char)root_data_block;
    printf("[search_dir_internal] got root_data_block_number = %d\n",
           root_data_block_number);
  }

  // get the data block for root directory if not assigned to variable
  // root_data_block yet
...
struct dir_entry *insert_dir(char file_name, char inode_number) {
  ...
  if (!dir_entry &&
      root_inode->length <
          BLOCK_SIZE / sizeof(struct dir_entry)) { // if not found and there is
                                                   // space to add an entry
